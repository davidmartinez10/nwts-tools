...
const { displayName, dependencies, devDependencies }
...
case "win32": {
  ...
  await $`Robocopy ${path.join(temp_folder, "node_modules/nw/nwjs")} ${
    app_directory} *.* /E /MOVE`;
  ...
  if (process.env.PACKAGE_TYPE === "plain") {
    ...
    if (with_dirs) {
      ...
      const script        = `(${archive_expander})()`.replace(
               "___NWTS-TOOLS_RESTORE_ORIGINAL_MAIN___", manifest.main);
      await fs.writeFile(path.join(build_directory, expander_path), script);

      manifest.main = expander_path;
    }

    await $`cd ${build_directory} && powershell Compress-Archive ".\\*" ${
      package_zip}`;
    await $`move ${package_zip} ${package_nw}`;

    if (process.env.PACKAGE_TYPE === "zip+exe") {
      await $`copy /b ${nw}+${package_nw} ${
        path.join(app_directory, `${application_name}.exe`)}`;
...
case "darwin": {
  await $`mv ${path.join(temp_folder, "/node_modules/nw/nwjs/nwjs.app/")} ${
    path.join(package_directory, application_name + ".app")}`;
  ...
  if (process.env.PACKAGE_TYPE === "plain") {
    ...
    await $`cd ${build_directory} && zip -r ${
      path.join("..", resources, "app.nw")} .`;
  }
...
case "linux": {
  await $`mv ${path.join(temp_folder, "/node_modules/nw/nwjs/")} ${
    path.join(package_directory, application_name)}`;

  if (process.env.PACKAGE_TYPE === "plain") {
    await $`cp -R ${build_directory} ${
      path.join(package_directory, application_name, "package.nw")}`;
  } else {
    await $`cd ${build_directory} && zip -r ${
      path.join("..", package_directory, application_name, "package.nw")} .`;
    if (process.env.PACKAGE_TYPE === "zip+exe") {
      await $`cd ${
        path.join(package_directory,
                  application_name)} && cat nw package.nw > ${
        application_name} && chmod +x ${application_name}`;
      await $`cd ${
        path.join(package_directory, application_name)} && rm nw package.nw`;
    }
  }
