...
export async function patch_nwjs_codecs(nw_path) {
  ...
  child_process.execSync(os.platform() === "win32"
                           ? `powershell Expand-Archive ${
                             ffmpeg_zip} -DestinationPath ${nw_path} -Force`
                           : `unzip -o ${JSON.stringify(ffmpeg_zip)} -d ${
                             JSON.stringify(nw_path)}`,
                         { stdio: "inherit" });
